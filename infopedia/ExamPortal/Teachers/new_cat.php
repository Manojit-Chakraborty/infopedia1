<?php
  ...
  if(isset($_POST['done']))
  {
    ...
    $orga=$_POST['done'];
    if($orga!=""){
      $sqlT="SELECT `email` FROM `student_login` WHERE `organization`='$orga'";
    $sql="INSERT INTO `category`(`catName`, `status`,`organization`) VALUES ('$name','0','$orga')";}
    else {
      $sqlT="SELECT `email` FROM `student_login` WHERE `organization`='ADMIN'";
    $sql="INSERT INTO `category`(`catName`, `status`) VALUES ('$name','0')";}
